  ...
  input_schema: {
    ...
    properties: {
      ...
      subject: {
        type: "string",
        description:
          "Patient full name (the person the record is about). Prefer the value entered for Patient name / Patient's name when that field clearly identifies the patient. If the form uses separate Given name and Surname (or First/Family/Last name) fields, join all patient parts with spaces in natural order (given name(s) first, then surname). If the document is clearly an email (From/To/Subject headers), the subject of care is usually the To recipient's display name—not the From sender unless the text shows otherwise. Names beginning with Dr, Doctor, or similar medical titles (including OCR DR) usually identify a clinician, not the patient—do not use them as subject unless the document explicitly states that person is the patient. Do not use a clinician as subject unless they are the patient.",
      },
